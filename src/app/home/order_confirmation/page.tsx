...
import { ShippingData } from "@/src/app/home/shipping/page";
            ...
            <div className="border rounded-lg p-6 mb-8 bg-gray-50">
              ...
              <div className="space-y-4 mb-6">
                {orderDetails.items.map((item: any) => (
                  <div key={item.phoneModelDetailsId} className="flex items-start gap-4 pb-4 border-b">
                    <div className="w-12 h-12 flex-shrink-0">
                      {item.image ? (
                        <img src={item.image} alt={item.titleName} className="w-full h-full object-contain" />
                      ) : (
                        <div className="w-12 h-12 bg-gray-100 rounded flex items-center justify-center text-xs text-gray-400">No img</div>
                      )}
                    </div>
                    <div className="flex-grow min-w-0">
                      <h3 className="text-sm font-medium truncate">{item.titleName}</h3>
                      <p className="text-xs text-gray-500">
                        {item.condition} - {item.storage} - {item.colorName}
                      </p>
